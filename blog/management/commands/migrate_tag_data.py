    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            if not blog_tag_exists:
                self.stdout.write(
                    "✅ blog_tag table doesn't exist - migration may already be complete"
                )
                return
            ...
            if conflicts:
                self.stdout.write(
                    f"⚠️  Found {len(conflicts)} tag names that exist in both tables:"
                )
            ...
            if not options["dry_run"]:
                ...
                with transaction.atomic():
                    ...
                    for old_id, name, slug in new_tags:
                        ...
                        self.stdout.write(
                            f"   ✅ Migrated tag '{name}': {old_id} -> {new_id}"
                        )
                    ...
                    for old_id, new_id in existing_mappings:
                        tag_mapping[old_id] = new_id
                        self.stdout.write(
                            f"   🔗 Mapped existing tag: {old_id} -> {new_id}"
                        )

                    # Update blog_entry_tags references
                    cursor.execute(
                        "SELECT COUNT(*) FROM blog_entry_tags WHERE tag_id IS NOT NULL"
                    )
                    ...
                    if entry_tag_count > 0:
                        ...
                        for old_id, new_id in tag_mapping.items():
                            ...
                            if updated > 0:
                                self.stdout.write(
                                    f"   ✅ Updated {updated} entry-tag references: {old_id} -> {new_id}"
                                )
                    ...
                    if blogmark_has_old_column:
                        cursor.execute(
                            "SELECT COUNT(*) FROM blog_blogmark_tags WHERE tag_id IS NOT NULL"
                        )
                        ...
                        if blogmark_tag_count > 0:
                            ...
                            for old_id, new_id in tag_mapping.items():
                                ...
                                if updated > 0:
                                    self.stdout.write(
                                        f"   ✅ Updated {updated} blogmark-tag references: {old_id} -> {new_id}"
                                    )
