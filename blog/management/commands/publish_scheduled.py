...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        entries_to_publish = Entry.objects.filter(
            ~Q(status="published"), publish_date__lte=now
        )
        ...
        for entry in entries_to_publish:
            if dry_run:
                self.stdout.write(
                    f"Would publish entry: {entry.title} (scheduled for {entry.publish_date})"
                )
            else:
        ...
        for blogmark in blogmarks_to_publish:
            if dry_run:
                self.stdout.write(
                    f"Would publish blogmark: {blogmark.title} (scheduled for {blogmark.publish_date})"
                )
            else:
