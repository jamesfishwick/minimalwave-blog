...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not hasattr(settings, "AZURE_STORAGE_ACCOUNT_NAME"):
            self.stdout.write(self.style.ERROR("❌ Azure Storage not configured!"))
            self.stdout.write(
    ...
    def check_entries(self, queryset, dry_run, force):
        ...
        if missing_count > 0:
            self.stdout.write(self.style.ERROR(f"    ❌ Missing: {missing_count}"))
            self.stdout.write(
                self.style.WARNING(
    ...
    def check_image(self, entry, dry_run, force):
        ...
        try:
            ...
            if entry.image.storage.exists(entry.image.name):
                self.stdout.write(f"    ✅ {entry.title[:50]}: {entry.image.name}")
                return "exists"
            else:
                self.stdout.write(
                    self.style.ERROR(
                ...
                self.stdout.write(f"       Entry ID: {entry.id}")
                self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(f"    ⚠️  {entry.title[:50]}: ERROR - {str(e)}")
            )
            return "error"
