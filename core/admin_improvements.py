...
class AdminIconMixin:
    ...
    def icon_preview(self, obj):
        if hasattr(obj, "icon") and obj.icon:
            return format_html(
                '<i class="{}" style="font-size: 16px; color: #666;"></i>', obj.icon
            )
        return "—"
...
class ContentStatusMixin:
    ...
    def status_badge(self, obj):
        if hasattr(obj, "status"):
            colors = {"draft": "#6B7280", "review": "#F59E0B", "published": "#10B981"}
            return format_html(
    ...
    def view_on_site_link(self, obj):
        if hasattr(obj, "get_absolute_url"):
            try:
                ...
                return format_html(
                    '<a href="{}" target="_blank" class="button">View</a>', url
                )
            except Exception:
                pass
        return "—"
...
class TaxonomyAdminMixin(AdminColorMixin, AdminIconMixin):
    ...
    class Media:
        css = {"all": ("admin/css/taxonomy_admin.css",)}
...
class MarkdownWidget(Textarea):
    ...
    def __init__(self, attrs=None):
        if attrs is None:
            attrs = {}
        attrs.update({"rows": 20, "cols": 100, "style": "font-family: monospace;"})
        super().__init__(attrs)

    class Media:
        css = {"all": ("admin/css/markdown_editor.css",)}
